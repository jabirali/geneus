...
contains
    ...
    !! those subroutines will be executed before/after each iteration of the update.
        ...
        if ((.not. bootstrap_) .or. selfconsistency) then
            ...
            do
                ...
                if ((materials .eq. 1) .and. (bootstrap_ .or. (.not. selfconsistency))) then
                    exit
                end if

                ! Exit criterion #2: minimum number of iterations reached,
                ! and the materials converged within specified parameters
                if ((n .ge. iterations_) .and. (this%difference() .lt. threshold_)) then
                    exit
                end if
            end do
        end if
    ...
    !! Constructs a multilayer stack from a configuration file.
        ...
        do while (iostat .eq. 0)
            ...
            i = scan(str, '{')
            j = scan(str, '}')
            do while (i .gt. 0 .and. j - 1 .ge. i + 1)
                ...
                if (status .ne. 0) then
                    write (arg, '(i0)') k + 1
                    call error('Missing command line argument #'//trim(arg)// &
                               ', i.e. parameter '//str(i:j)//' in the config file.')
                end if
                if (scan(trim(arg), '+-') .eq. 1) then
